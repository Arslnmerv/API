...
Dönen response un Status kodunun 201 ve response body nin aşağıdaki gibi olduğunu test edin
   {
    ...
    public void test() {
        ...
        assertEquals(expectedRequest.getString("title"), actualDataMap.get("title"));
        assertEquals(expectedRequest.getInt("userId"), actualDataMap.get("userId"));
        assertEquals(expectedRequest.getBoolean("completed"), actualDataMap.get("completed"));
